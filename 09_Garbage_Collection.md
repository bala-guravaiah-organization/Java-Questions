...
```
---
#### Image Representation for Young Generation and Old Generation :
![Mark and Sweep Algoritham Working](Images/Garbage_Collection_Mark_And_Sweep_Algoritham.png)
...
![Heap Explanation](Images/Garbage_Collector_Heap_Explanation.png)
...
#### **Comparison Table**
| **Garbage Collector** | **Best For** | **Pause Time** | **Heap Size** |
|----------------------|-------------|---------------|--------------|
| **Serial GC** | Small apps, single-threaded programs | High | Small |
| **Parallel GC** | High-throughput apps, batch processing | Medium | Medium to Large |
| **CMS GC** (Deprecated) | Low-latency apps (before Java 9) | Low | Medium to Large |
| **G1 GC** (Default in Java 9+) | General-purpose, balanced performance | Low | Large |
| **ZGC** | Large heap, ultra-low-latency apps | Very Low (<10ms) | Huge (TB-scale) |
| **Shenandoah GC** | Low-latency workloads | Very Low | Medium to Large |
...
#### **What are the Differences Between Serial and Parallel Garbage Collectors?**
| Feature | Serial Garbage Collector | Parallel Garbage Collector |
|---------|-------------------------|----------------------------|
| **Thread Usage** | Uses a **single thread** | Uses **multiple threads** |
| **Best For** | Small applications, single-core CPUs | Large applications, multi-core CPUs |
